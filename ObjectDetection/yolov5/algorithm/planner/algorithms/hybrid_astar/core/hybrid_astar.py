...
def calc_next_node(n_curr, c_id, u, d, P):
    ...
    yawlist = [rs.pi_2_pi(n_curr.yaw[-1] + d * C.MOVE_STEP / Car_C.WB * math.tan(u))]

    for i in range(nlist - 1):
        xlist.append(xlist[i] + d * C.MOVE_STEP * math.cos(yawlist[i]))
        ylist.append(ylist[i] + d * C.MOVE_STEP * math.sin(yawlist[i]))
        yawlist.append(rs.pi_2_pi(yawlist[i] + d * C.MOVE_STEP / Car_C.WB * math.tan(u)))
...
def analystic_expantion(node, ngoal, P):
    ...
    maxc = math.tan(Car_C.MAX_STEER) / Car_C.WB
...
def is_collision(x, y, yaw, P):
    for ix, iy, iyaw in zip(x, y, yaw):
        d = 0.8
        dl = (Car_C.RF - Car_C.RB) / 2.0
        r = (Car_C.RF + Car_C.RB) / 2.0 + d
        ...
        for i in ids:
            ...
            if abs(dx) < r and abs(dy) < Car_C.W / 2 + d:
                return True
...
def calc_rs_path_cost(rspath):
    ...
    for ctype in rspath.ctypes:
        if ctype != "S":
            cost += C.STEER_ANGLE_COST * abs(Car_C.MAX_STEER)
