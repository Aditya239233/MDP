    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            if(s==1):
                ...
                w=int(width)
                h=int(height)
                cap = cv2.dnn.blobFromImage(img,0.00392,(320,320),(0,0,0),True,crop=False) #reduce 416 to 320    
                image_hub.send_reply(b'OK')
                self.fps[i] =  30.0  # 30 FPS fallback
                self.frames[i] = float('inf')
                self.imgs[i] = img  # guarantee first frame
                self.threads[i] = Thread(target=self.updatepi, args=([i, image_hub]), daemon=True)
            else:
